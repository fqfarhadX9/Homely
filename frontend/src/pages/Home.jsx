...
function Home() {
  const {listingData, newListingData} = useContext(ListingDataContext);
  return (
    <div>
      <Nav/>
      <div className='w-[100vw] h-[77vh] flex items-center justify-center gap-[25px] flex-wrap mt-[250px] md:mt-[180px] '>
        {newListingData.map((list) => (
          ...
          image3={list.image3} rent={list.rent} id={list._id} ratings={list.ratings} isBooked={list.isBooked} host={list.host}/>
        ))}
      </div>
    </div>
  )
